...
const PARAMETERS: Parameters[] = [
  { id: 1, name: 'Population', amount: 300, time: 1, count: "<a href='javascript:;'>Clone yourself!</a>" }
];
...
//  kpis = KPIS; 
  ...
  updateBuildings(parameter : Parameters){
    if(parameter.id === 1 && parameter.amount>=750){
        this.router.navigateByUrl('/win');
    } else if (parameter.id === 1 && this.killedPeople >= 1000){
  ...
  getMoreThings(){
    ...
    } else if (PARAMETERS[0].amount > 75 && !this.buildings[4]){
      UPGRADES.push({id: 1, name: "Improved Cloning Facilities", purchased: 0, cost: 75, cost_id: 0, buytext: "<a href='javascript:;'>Sacrifice 100 villagers to improve your cloning facilities!</a>"});
